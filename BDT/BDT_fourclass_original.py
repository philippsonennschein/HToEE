...
vbh_sum_w = train_w_df[train_w_df['proc'] == 'VBH']['weight'].sum()
...
train_w_df.loc[train_w_df.proc == 'VBH','weight'] = (train_w_df[train_w_df['proc'] == 'VBH']['weight'] * ggh_sum_w / vbh_sum_w)
...
def roc_score(y_true = y_true, y_pred = y_pred_test):
    ...
    print("Area under ROC curve for VBH (test): ", auc_keras_test_vbf)
    ...
    ax.plot(fpr_keras_vbf, tpr_keras_vbf, label = 'VBH (area = %0.2f)'%auc_keras_test_vbh)
...
def plot_output_score(data='output_score_vbf', density=False,):
    ...
    ax.hist(output_score_vbh, bins=50, label='VBH', histtype='step',weights=vbh_w) #density=True)
...
plot_output_score(data='output_score_vbh')
