...
class Plotter(object):
    ...
    def plot_input(self, var, n_bins, out_label, ratio_plot=False, norm_to_data=False, extra_cuts=None, extra_tag=None, blind=False):
        ...
        if ratio_plot:
            ...
            fig, axes = plt.subplots(nrows=2, ncols=1, dpi=200, sharex=True, gridspec_kw ={'height_ratios':[3,0.8], 'hspace':0.08})  
            ratio = axes[1]
            axes = axes[0]
        else:
            fig  = plt.figure(1)
        ...
        np.savetxt('models/var_sig.csv', var_sig, delimiter=',')
        ...
        if norm_to_data:
            ...
            np.savetxt('models/bkg_stack.csv', bkg_stack, delimiter=',')
            np.savetxt('models/bkg_stack_concat.csv', np.concatenate(bkg_stack), delimiter=',')
    ...
    def plot_roc(self, y_train, y_pred_train, train_weights, y_test, y_pred_test, test_weights, out_tag='MVA'):
        ...
        np.savetxt('models/bkg_eff_train.csv', bkg_eff_train, delimiter=',')
        np.savetxt('models/sig_eff_train.csv', sig_eff_train, delimiter=',')
    ...
    def plot_output_score(self, y_test, y_pred_test, test_weights, proc_arr_test, data_pred_test, MVA='BDT', ratio_plot=False, norm_to_data=False, log=False):        
        ...
        np.savetxt('models/data_pred_test.csv', data_pred_test, delimiter=',')

        axes.hist(sig_scores, bins=bins, label=self.sig_labels[0]+r' ($\mathrm{H}\rightarrow\gamma\gamma$) '+self.num_to_str(self.sig_scaler), weights=sig_w_true*(self.sig_scaler), density=True, histtype='step', color=self.sig_colour)
        ...
        if norm_to_data:
            ...
            for w_arr in bkg_w_stack:
                rew_stack.append(w_arr*k_factor)
            bkg_mc_binned, _, _ = axes.hist(bkg_stack, bins=bins, label=bkg_proc_stack, weights=rew_stack, histtype='step', color='blue', stacked=True, zorder=0) #self.bkg_colours[0:len(bkg_proc_stack)]
            bkg_stack_summed, _ = np.histogram(np.concatenate(bkg_stack), bins=bins, weights=np.concatenate(rew_stack))
            sumw2_bkg, _  = np.histogram(np.concatenate(bkg_stack), bins=bins, weights=np.concatenate(rew_stack)**2)

            np.savetxt('models/bkg_stack2.csv', bkg_stack, delimiter=',')
            np.savetxt('models/bkg_stack_concat2.csv', np.concatenate(bkg_stack), delimiter=',')

        else:
            axes.hist(bkg_stack, bins=bins, label=bkg_proc_stack, weights=bkg_w_stack, histtype='step', density=True, color= 'blue', stacked=True, zorder=0) #self.bkg_colours[0:len(bkg_proc_stack)]
            bkg_stack_summed, _ = np.histogram(np.concatenate(bkg_stack), bins=bins, weights=np.concatenate(bkg_w_stack))
            sumw2_bkg, _  = np.histogram(np.concatenate(bkg_stack), bins=bins, weights=np.concatenate(bkg_w_stack)**2)
