...
for i in range(len(signal)):
    ...
    fpr_keras.sort()
    tpr_keras.sort()
    name_fpr = 'csv_files/VH_NN_binary_fpr_' + signal[i]
    name_tpr = 'csv_files/VH_NN_binary_tpr_' + signal[i]
...
ax.grid(True, 'major', linestyle='dotted', color='grey', alpha=0.5)
plt.tight_layout()
name = 'plotting/BDT_plots/NN_VH_binary_Multi_ROC_curve'
plt.savefig(name, dpi = 1200)
print("Plotting ROC Curve")
plt.close()
...
name_cm = 'csv_files/VH_NN_binary_cm'
